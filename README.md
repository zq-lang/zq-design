...
- [x] _Unlike JavaScript_: [three equals signs are too many](syntax/triple-equals.md) (`===` and `!==`)
...
- [x] _Unlike JavaScript_: [`const` variables do not need to be immediately initialized](syntax/const-initialization.md)
...
- [x] _Unlike JavaScript_: [there is no need for `undefined` and `null`](type-system/undefined-or-null.md) (get rid of `undefined`)
- [x] _Unlike JavaScript_: [there is no need for `instanceof` and `typeof`](type-system/instanceof-or-typeof.md) (get rid of `typeof`)
...
- [x] _Unlike Java_: [comparing comparables with `<`/`>`/`<=`/`>=` should be allowed](type-system/comparable-operators.md)
...
- [x] _Unlike Java_: [types should be able to define multiple equality or comparison schemes](standard-library/equator.md)
